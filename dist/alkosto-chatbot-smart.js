...
async function smartAlkostoChatbot(userInput) {
    ...
    try {
        ...
        if (data.success && data.productos.length > 0) {
            ...
            if (brandRequested) {
                const brandFiltered = filteredProducts.filter((p) => p.brand.toLowerCase().includes(brandRequested.toLowerCase()) ||
                ...
                if (brandFiltered.length > 0) {
                    ...
                    const availableBrands = [...new Set(data.productos.map((p) => p.brand))];
            ...
            filteredProducts.slice(0, 3).forEach((producto, index) => {
...
function detectCategory(input) {
    const lowerInput = input.toLowerCase();
    if (lowerInput.includes("televisor") || lowerInput.includes("tv"))
        return "televisor";
    if (lowerInput.includes("celular") || lowerInput.includes("móvil") || lowerInput.includes("smartphone"))
        return "celular";
    if (lowerInput.includes("computador") || lowerInput.includes("laptop") || lowerInput.includes("portátil"))
        return "computador";
    if (lowerInput.includes("lavadora"))
        return "lavadora";
