...
import { Product } from "./product-loader.js";
...
private async createNewIndex(products: Product[]): Promise<void> {
  ...
  private getPriceCategory(price: number): string {
    if (price < 500000) return "económico barato accesible";
    if (price < 1000000) return "precio medio moderado";
    if (price < 2000000) return "precio alto premium";
    if (price < 5000000) return "costoso caro exclusivo";
  ...
  async searchSimilar(
    ...
    try {
      ...
      const results: VectorSearchResult[] = searchResults
        .map(([doc, score]) => {
          const product = doc.metadata.originalProduct as Product;
...
private enhanceQuery(query: string): string {
  ...
  private keywordSearch(query: string, products: Product[], maxPrice?: number): Product[] {
  ...
  private calculateKeywordScore(query: string, product: Product): number {
    const queryTerms = query.toLowerCase().split(/\s+/);
    const searchText = this.createSearchText(product);
  ...
  getStats(): { isInitialized: boolean; indexPath: string } {
    return {
