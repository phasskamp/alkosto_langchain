...
async function analyzeSamsungInCSV() {
  ...
  try {
    ...
    const samsungProducts = parsed.data.filter((product: any) => {
    ...
    if (samsungProducts.length > 0) {
      ...
      samsungProducts.slice(0, 10).forEach((product: any, index: number) => {
    ...
    const tvProducts = parsed.data.filter((product: any) => {
      const title = (product.title || '').toLowerCase();
    ...
    tvProducts.forEach((product: any) => {
      if (product.brand) {
    ...
    const samsungTVs = tvProducts.filter((product: any) => {
      const title = (product.title || '').toLowerCase();
      const brand = (product.brand || '').toLowerCase();
    ...
    if (samsungTVs.length > 0) {
      console.log("\n📺 SAMSUNG TVS FOUND:");
      samsungTVs.forEach((tv: any, index: number) => {
